...
my $year = shift(@ARGV) || die "usage: $0 YYYY\n";;
if ($year !~ m#^\d\d\d\d$#) {
    die "usage: $0 YYYY\n";;
}
my $start = "01/01/$year";
...
my $ret = system("curl --silent --cookie-jar $cookie -H 'Accept: application/json' --output $json --dump-header $header '$osti\?entry_date_start=$start\&entry_date_end=$end'");
if ($ret) {
