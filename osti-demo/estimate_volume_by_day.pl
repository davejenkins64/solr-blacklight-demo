...
while ($now > $then) {
    ...
    my $ret = system("curl --silent --cookie-jar cookie.jar -H 'Accept: application/json' --output records.json --dump-header records.headers 'https://www.osti.gov/api/v1/records\?entry_date_start=$start\&entry_date_end=$start'");
    if ($ret) {
    ...
    if (($pages > 0) && ($size > 0)) {
        my $kb = ($pages * $size)/(1024);
        print "$start $pages $size = " . int($kb) . " KB\n";    
        $total_pages += $pages;
        $total += $kb;
    }
    else {
        print "$start 0 0 = 0 KB\n";
    }

    # do the previous day
    $now -= 24 * 3600;
}
print "TOTAL $total_pages pages and $total KB\n";
